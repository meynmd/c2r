...
class ConvClassifier(nn.Module):
    def __init__(self, num_categories, batch_size, window_size, use_cuda=None):
        ...
        flat_size = 256*(128//2**4)*(window_size//2**5)

        self.fc1_new = nn.Linear(flat_size, 1024)
        self.fc2_new = nn.Linear(1024, 128)
        self.fc3_new = nn.Linear(128, num_categories)
    ...
    def forward(self, input):
        ...
        features = F.relu(self.batchnorm_new(self.conv6_new(features)))

        flat = features.view(features.shape[0], -1)
        out = F.relu(self.batchnorm1d_new(self.fc1_new(flat)))
        out = F.relu(self.fc2_new(out))
        return self.fc3_new(out)
