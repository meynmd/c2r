...
class Encoder(nn.Module):
    def __init__(self, num_categories, batch_size, rnn_size=2048, num_rnn_layers=3, use_cuda=None, max_w=None):
        ...
        self.conv1 = nn.Conv2d(1, 32, (5, 9), padding=(2, 4), stride=(1, 1))
        self.maxpool_square = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))
        self.conv2 = nn.Conv2d(32, 64, (3, 5), padding=(1, 2), stride=(1, 1))
        self.conv3 = nn.Conv2d(64, 128, (3, 3), padding=(1, 1), stride=(1, 1))
        self.batchnorm128 = nn.BatchNorm2d(128)
        self.conv4 = nn.Conv2d(128, 128, (3, 3), padding=(1, 1), stride=(1, 1))
        ...
        self.rnn = nn.LSTM(16384, rnn_size, num_layers=self.rnn_layers, bidirectional=False)

        self.fc1 = nn.Linear(rnn_size, rnn_size // 2)
        self.fc2 = nn.Linear(rnn_size // 2, 256)
        self.fc3 = nn.Linear(256, num_categories)
        self.batchnorm1d = nn.BatchNorm1d(rnn_size // 2)
    ...
    def forward(self, input):
        ...
        features = F.relu(self.batchnorm128(self.conv3(features)))
        features = self.maxpool_wide(features)
        features = self.maxpool_wide(F.relu(self.conv4(features)))
        features = self.maxpool_wide(F.relu(self.conv5(features)))
