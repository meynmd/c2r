...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        # max_w = min(self.max_w, min(t.data.shape[1] for t in input))
        max_w = min(input.data.shape[1], self.max_w)
        x_batch = input
        # for tensor in input:
        #     tensor = tensor.view(1, 1, tensor.data.shape[0], tensor.data.shape[1])
        #     if tensor.data.shape[3] > max_w:
        #         tensor = self.random_crop(tensor, max_w)
        #     x_batch.append(tensor)

        # x_batch = torch.FloatTensor(1, 1, input.data.shape[0], input.data.shape[1])
        # x_batch[0,0,:,:] = input.data
